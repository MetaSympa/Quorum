...
export default function SponsorshipPage() {
  ...
  const fetchSponsors = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        const data: PaginatedSponsors = await res.json();
  ...
  const fetchLinks = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/sponsor-links?${params}`);
      if (res.ok) {
  ...
  useEffect(() => { if (status === "authenticated") fetchSponsors(); }, [status, fetchSponsors]);
  useEffect(() => { if (status === "authenticated") fetchLinks(); }, [status, fetchLinks]);
  ...
  async function handleGenerateLink() {
    ...
    try {
      ...
      const res = await fetch("/api/sponsor-links", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDeactivateLink(link: SponsorLink) {
    ...
    const res = await fetch(`/api/sponsor-links/${link.token}`, { method: "PATCH" });
    const data = await res.json();
    if (!res.ok) {
