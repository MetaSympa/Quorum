/**
 * T34 — Additional unit tests for src/lib/utils.ts
 *
 * Covers edge cases not in the original utils.test.ts:
 *   - cn() class merging utility
 *   - formatCurrency with edge values
 *   - formatDate with Date objects at year boundaries
 *   - formatDateTime with various times
 *   - formatPhone edge cases
 *   - formatMembershipType with unknown/fallback values
 *   - formatMembershipStatus via toTitleCase for all known statuses
 *   - formatSponsorPurpose for all enum values
 */
